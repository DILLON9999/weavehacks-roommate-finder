...
export class HousingAgentMCP extends MCPBaseAgent {
  ...
  private async loadHousingData(): Promise<void> {
    try {
      // Load Craigslist data with images
      const craigslistPath = join(process.cwd(), 'CraigslistData', 'clean-listings.json');
      const craigslistImagesPath = join(process.cwd(), 'CraigslistData', 'images.json');
      const craigslistRaw = readFileSync(craigslistPath, 'utf-8');
      const craigslistImagesRaw = readFileSync(craigslistImagesPath, 'utf-8');
      
      const craigslistListings = JSON.parse(craigslistRaw);
      const craigslistImages = JSON.parse(craigslistImagesRaw);
      
      this.craigslistData = craigslistListings.map((item: CleanListing) => ({
        ...item,
        source: 'craigslist',
        images: craigslistImages[item.id || ''] || []
      }));
      console.log(`📊 Loaded ${this.craigslistData.length} Craigslist listings with images`);

      // Load Facebook data with images
      const facebookPath = join(process.cwd(), 'FacebookData', 'clean-listings.json');
      const facebookImagesPath = join(process.cwd(), 'FacebookData', 'images.json');
      const facebookRaw = readFileSync(facebookPath, 'utf-8');
      const facebookImagesRaw = readFileSync(facebookImagesPath, 'utf-8');
      
      const facebookListings = JSON.parse(facebookRaw);
      const facebookImages = JSON.parse(facebookImagesRaw);
      
      this.facebookData = facebookListings.map((item: CleanListing) => ({
        ...item,
        source: 'facebook',
        images: facebookImages[item.id || ''] || []
      }));
      console.log(`📊 Loaded ${this.facebookData.length} Facebook listings with images`);

      // Log image statistics
      const craigslistWithImages = this.craigslistData.filter(item => item.images && item.images.length > 0);
      const facebookWithImages = this.facebookData.filter(item => item.images && item.images.length > 0);
      console.log(`📷 Images loaded: ${craigslistWithImages.length}/${this.craigslistData.length} Craigslist, ${facebookWithImages.length}/${this.facebookData.length} Facebook`);
  ...
  private async searchHousing(query: string, filters: DeterministicFilters = {}, maxResults: number = 5): Promise<MatchedListing[]> {
    ...
    const filteredListings = this.filterHousing(filters);
