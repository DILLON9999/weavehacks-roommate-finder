...
exports.testHousingAgent = testHousingAgent;
const base_agent_1 = require("./base-agent");
const fs_1 = require("fs");
const dotenv_1 = require("dotenv");
(0, dotenv_1.config)();
class HousingAgent extends base_agent_1.BaseAgent {
    constructor() {
        super('HousingAgent');
        this.listings = [];
    ...
    async handleExtractFilters(message) {
        try {
            const { query } = message.payload;
    ...
    async handleGetListingsCount(message) {
        return {
            success: true,
            data: {
                totalListings: this.listings.length,
                craigslistListings: this.listings.filter(l => l.source === 'craigslist').length,
                facebookListings: this.listings.filter(l => l.source === 'facebook').length
            },
            confidence: 1.0
        };
    }
...
`;
    ...
    getSummary() {
        ...
        const craigslistCount = this.listings.filter(l => l.source === 'craigslist').length;
        const facebookCount = this.listings.filter(l => l.source === 'facebook').length;
...
async function testHousingAgent() {
