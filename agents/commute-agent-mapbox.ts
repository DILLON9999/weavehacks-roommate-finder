import { BaseAgent, AgentCapability, AgentResponse, AgentMessage } from './base-agent';
...
export class MapboxCommuteAgent extends BaseAgent {
  ...
  private setupMessageHandlers(): void {
    this.registerHandler('analyze_commute', async (message: AgentMessage) => {
      const request = message.payload as CommuteRequest;
    ...
    this.registerHandler('get_route', async (message: AgentMessage) => {
  ...
  async analyzeCommute(request: CommuteRequest): Promise<CommuteAnalysis | null> {
    if (!this.mcpClient) {
...
`;
      ...
      const analysisContent = analysisResponse.content as string;
  ...
  async getRoute(origin: string, destination: string, travelMode: string = 'driving-traffic'): Promise<any> {
    if (!this.mcpClient) {
      return {
        source: 'mock_data',
        origin,
